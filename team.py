...
class Team():
    ...
    def acquirePlanet(self, planets, index, population):
        self.planets[index] = planets[index]
        planets[index].addToTeam(self)
        planets[index].addPopulation(population)
        self.spriteGroup.add(planets[index])
        self.getCommsNetwork()
    
    def acquireShip(self, ship, location):
        try:
            self.ships[ship.type].append(ship)
        except KeyError: self.ships[ship.type] = [ship]
    ...
    def getCommsNetwork(self):
        ...
        for s in self.spriteGroup.sprites():
            ...
            if s in final: s.comms = True
            else: 
                s.comms = False
        if final != initial:
            self.commsNetwork = final
            if self.index == 0:
                self.u.getFogBase()
        finalVis = []
        finalVis.extend(final)
        for s in final:
            vis = self.getVis(s)
            for v in vis:
                if v not in finalVis: finalVis.append(v)
        self.visible = finalVis
    ...
    def getVis(self, base):
        '''Return a list of sprites from other teams seen by the base sprite'''
        vis = []
    ...
    def removeFromAG(self, list):
        ...
        captain: list of one ship, ships: list
                ...
                if len(threats) > 0:
                    ...
                    if len(threat_rank) > 0:
                        ...
                        for i in range(len(sorted_ships)):
                            try:
                                sorted_ships[i].baseTarget[1]/1    # guarantee it's just a y-coord
                                targ1 = sorted_ships[i].baseTarget[0], sorted_ships[i].baseTarget[1]
                                    # replace with coordinates to move to after battle
                            except TypeError:           # could be a list
                                if len(sorted_ships[i].baseTarget[1]) < 2:
                                ...
                                targ1 = sorted_ships[i].baseTarget[1]       #leave as is
